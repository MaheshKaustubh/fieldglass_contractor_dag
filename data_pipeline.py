...
def check():
    ...
    wds=[datetime(2024, 1, 2).date(), datetime(2024, 1, 3).date(), datetime(2024, 1, 4).date(), datetime(2023, 11, 2).date(), datetime(2023, 12, 4).date(), datetime(2023,12,17).date(), datetime(2023,12,18).date()]
...
# C:/Users/6126176/AppData/Local/Packages/CanonicalGroupLimited.Ubuntu_79rhkp1fndgsc/LocalState/rootfs/home/kaustubhmahesh/airflow/CONSOLIDATED_WORKER_HEADCOUNT_TEST.csv
    if os.path.exists(file_path):
        try: 
            sfconnector.execute(f"put file://{file_path} @DATA_LOAD_STAGE")
        except Exception as e: 
            print(e)
    else: 
